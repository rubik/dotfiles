...
" 
noremap <unique> <script> <Plug>MiniBufExplorer  :call <SID>StartExplorer(1, -1, 
bufnr("%"))<CR>:<BS>
noremap <unique> <script> <Plug>CMiniBufExplorer :call <SID>StopExplorer(1)<CR>:<BS>
noremap <unique> <script> <Plug>UMiniBufExplorer :call 
<SID>AutoUpdate(-1,bufnr("%"))<CR>:<BS>
noremap <unique> <script> <Plug>TMiniBufExplorer :call <SID>ToggleExplorer()<CR>:<BS>
noremap <unique> <script> <Plug>MBEMarkCurrent :call 
<SID>MarkCurrentBuffer(bufname("%"),1)<CR>:<BS>
...
if g:miniBufExplUseSingleClick == 1
  let s:clickmap = ':if bufname("%") == "-MiniBufExplorer-" <bar> call <SID>MBEClick() <bar> 
endif <CR>'
  if maparg('<LEFTMOUSE>', 'n') == '' 
    ...
    let  s:m = s:m . substitute(substitute(maparg('<LEFTMOUSE>', 'n'), '|', '<bar>', 'g'), 
'\c^<LEFTMOUSE>', '', '')
...
<SID>AutoUpdate(expand('<abuf>'),bufnr("%"))
autocmd MiniBufExplorer BufDelete      * call <SID>DEBUG('-=> BufDelete ModTrackingListClean 
AutoCmd for buffer '.bufnr("%"), 10) |call <SID>CleanModTrackingList(bufnr("%"))
autocmd MiniBufExplorer BufEnter       * call <SID>DEBUG('-=> BufEnter AutoCmd', 10) |call 
<SID>AutoUpdate(-1,bufnr("%"))
autocmd MiniBufExplorer BufEnter       * call <SID>DEBUG('-=> BufEnter Checking for Last 
window', 10) |call <SID>CheckForLastWindow()
autocmd MiniBufExplorer BufWritePost   * call <SID>DEBUG('-=> BufWritePost AutoCmd', 10) 
|call <SID>AutoUpdate(-1,bufnr("%"))
autocmd MiniBufExplorer CursorHold     * call <SID>DEBUG('-=> CursroHold AutoCmd', 10) |call 
<SID>AutoUpdateCheck(bufnr("%"))
autocmd MiniBufExplorer CursorHoldI    * call <SID>DEBUG('-=> CursorHoldI AutoCmd', 10) 
|call <SID>AutoUpdateCheck(bufnr("%"))
autocmd MiniBufExplorer VimEnter       * call <SID>DEBUG('-=> VimEnter AutoCmd', 10) |let 
g:miniBufExplorerAutoUpdate = 1 |call <SID>AutoUpdate(-1,bufnr("%"))
augroup NONE
" }}}
...
function! CheckRootDirForDupes(level,path1,path2)
    call <SID>DEBUG('Entering Dupe Dir Checking Function for at level '.a:level.' for 
'.join(a:path1).' vs '.join(a:path2),10)
    ...
    while(l:i <= l:NBuffers)
        ...
        if (g:miniBufExplCheckDupeBufs == 1)
            while(l:i2 <= l:NBuffers)
                ...
                call <SID>DEBUG('BUFFER PATHS ---> bufPath is '.l:bufPath.' and bufPath2 is 
'.l:bufPath2,10)
                call <SID>DEBUG('BUFFER NAMES ---> bufName is '.l:BufName.' and BufName2 is 
'.l:BufName2,10)
                ...
                if((l:BufName2 != '') && (l:bufPath != l:bufPath2))
                    call <SID>DEBUG('bufPath2 is not empty and not comparing the same file, 
going to check for dupes!',10)
                    ...
                    if(strlen(l:BufName))
                        ...
                        call <SID>DEBUG('Setting bufFileNameFromPath as 
'.l:bufSplitPath[-1],10)
                    endif
                    ...
                    if(strlen(l:BufName2))
                        ...
                        call <SID>DEBUG('Setting bufFileNameFromPath2 as 
'.l:bufSplitPath2[-1],10)
                    endif
            ...
            " function that show a differentiating parent directory so that the name is 
unique.
            if l:dupeBufName >= 1
                for item in l:pathList
                    call <SID>DEBUG('Item in pathList loop is '.item,10)
                    if ((!empty(item)) && (item != l:bufPath))
                        call <SID>DEBUG('2 or more duplicate buffer names, calling dir check 
function with '.l:bufPath.' vs '.item,10)
                        call 
CheckRootDirForDupes(s:bufPathPosition,split(l:bufPath,s:PathSeparator,0),split(item,s:PathSeparator,0))
                    endif
                endfor
            endif
        endif
...
function! <SID>HasEligibleBuffers(delBufNum)
  ...
  call <SID>DEBUG('HasEligibleBuffers found '.l:found.' eligible buffers of '.l:needed.' 
needed',6)
...
'.bufname('%'),10)
  ...
  if (a:delBufNum != -1)
    call <SID>DEBUG('AutoUpdate will make sure that buffer '.a:delBufNum.' is not included 
in the buffer list.', 5)
...
function! <SID>MBESelectBuffer(split)
  ...
  if(l:bufnr != -1)             " If the buffer exists.
    ...
    if bufname('%') == '-MiniBufExplorer-' || (g:miniBufExplModSelTarget == 1 && 
...
function! <SID>MBEDeleteBuffer(prevBufName)
  ...
  if l:selBufName == 'MiniBufExplorer.DBG' && g:miniBufExplorerDebugLevel > 0
    call <SID>DEBUG('MBEDeleteBuffer will not delete the debug window, when debugging is 
turned on.',1)
    return
  endif
  ...
  if l:selBuf != -1 
    ...
    while l:winNum != -1 
        ...
        call <SID>DEBUG('We are now in window: '.winnr().' which contains buffer: 
'.bufnr('%').' and should contain buffer: '.l:selBuf,5)

        let l:origBuf = bufnr('%')
        call <SID>CycleBuffer(1)
        let l:curBuf  = bufnr('%')

        call <SID>DEBUG('Window now contains buffer: '.bufnr('%').' which should not be: 
'.l:selBuf,5)
  ...
  if g:miniBufExplorerDebugLevel >= a:level
    ...
    if g:miniBufExplorerDebugMode == 0
        ...
        if bufname('%') != 'MiniBufExplorer.DBG'
            call confirm('Error in window debugging code. Dissabling MiniBufExplorer 
debugging.', 'OK')
    ...
    elseif g:miniBufExplorerDebugMode == 2
        if has('system') || has('fork')
            if has('win32') && !has('win95')
                let l:result = system("cmd /c 'echo ".s:debugIndex.':'.a:level.':'.a:msg." 
>> MiniBufExplorer.DBG'")
            endif
            if has('unix')
                let l:result = system("echo '".s:debugIndex.':'.a:level.':'.a:msg." >> 
MiniBufExplorer.DBG'")
            endif
        else
            call confirm('Error in file writing version of the debugging code, vim not 
compiled with system or fork. Dissabling MiniBufExplorer debugging.', 'OK')
            let g:miniBufExplorerDebugLevel = 0
        endif
